...
template <is_nav2_controller T> class SpinningController : public T {
public:
  void configure(
    ...
    nav2_util::declare_parameter_if_not_declared(
        node, plugin_name_ + ".spin_period", rclcpp::ParameterValue(5.0));
    node->get_parameter(plugin_name_ + ".spin_period", cmds_->period);

    nav2_util::declare_parameter_if_not_declared(
        node, plugin_name_ + ".spin_commands", rclcpp::PARAMETER_DOUBLE_ARRAY);
    auto vec_d =
      ...
      // Spin commands - individual spin orders
        ...
        if (name == plugin_name_ + ".spin_commands") {
          ...
          for (const auto &cmd : cmds_->commands) {
            RCLCPP_INFO(logger_, "Omega: %f, Duration: %f", cmd.omega,
                        cmd.duration);
          }
        }
      }
...
private:
  void maybe_spin(geometry_msgs::msg::TwistStamped &cmd_vel) {
    // TODO: Somewhere assert that sum of instruction durations is less than
    // total duration using LinearVec3 = decltype(cmd_vel.twist.linear);
    ...
    constexpr auto has_linear = [](auto &cmd_vel) {
      static constexpr auto epsilon = 0.01;
      return std::abs(cmd_vel.twist.linear.x) > epsilon ||
             std::abs(cmd_vel.twist.linear.y) > epsilon ||
    ...
    if (delta > cmds_->period && has_linear(cmd_vel)) {
